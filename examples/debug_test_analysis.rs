...
fn main() {
    ...
    println!("=== Analyzing Board Position for Iteration {} ===", i);
    ...
    println!("Is ({}, 5) empty? {}", base_row, state.board.is_empty_position(base_row, 5));
    println!("Is ({}, 10) empty? {}", base_row, state.board.is_empty_position(base_row, 10));
    println!("Is (6, 5) empty? {}", state.board.is_empty_position(6, 5));
    ...
    // Test if (6,5) is also winning (which would explain the confusion)
    let mut test_state3 = state.clone();
    test_state3.make_move((6, 5));
    println!("After playing (6, 5): terminal = {}, winner = {:?}", 
             test_state3.is_terminal(), test_state3.winner);
    
    // See what the AI actually chooses multiple times
    println!("\n=== AI Decision (Multiple Runs) ===");
    for run in 0..5 {
        let mut test_state = state.clone();
        let best_move = find_best_move_parallel(&mut test_state, 6, None, &shared_tt);
        println!("Run {}: AI chose: {:?}", run, best_move);
    }
}
