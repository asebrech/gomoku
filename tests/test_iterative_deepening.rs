...
fn test_find_best_move_with_time_limit() {
    ...
    let result = lazy_smp_search(&mut state, 50, 10, Some(1));
    assert!(result.best_move.is_some());
...
fn test_iterative_deepening_vs_direct_minimax() {
    ...
    let iterative_result = lazy_smp_search(&mut state, 100, 10, Some(1));
    
    // Test direct search using the regular find_best_move function  
    let regular_result = lazy_smp_search(&mut state, 100, 10, Some(1));
...
fn test_complex_board_with_short_time_limit() {
    ...
    let opponent_moves = state.order_moves();
    ...
    let result = lazy_smp_search(&mut state, 500, 10, Some(1));
...
fn test_time_vs_depth_consistency() {
    ...
    let short_result = lazy_smp_search(&mut state, 50, 10, Some(1));
    
    // Test with generous time limit that should allow reaching deeper
    let long_result = lazy_smp_search(&mut state, 5000, 10, Some(1));
...
fn test_transposition_table_benefits() {
    ...
    let first_result = lazy_smp_search(&mut state, 100, 10, Some(1));
    
    // Second search should benefit from transposition table
    let second_result = lazy_smp_search(&mut state, 100, 10, Some(1));
